...
import { getRandomWord, getSpeechLang, getAvailableLevels, type WordEntry } from '../data/dailyWords';
...
export const DailyWordBadge: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // 获取新词
        setWord(getRandomWord(language, newLevel));
    }, [language]);

    // 监听难度变化，获取新词
    useEffect(() => {
        localStorage.setItem(`daily_word_level_${language}`, level);
        setWord(getRandomWord(language, level));
    }, [level, language]);
