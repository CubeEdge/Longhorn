...
const FileBrowser: React.FC<FileBrowserProps> = ({ mode = 'all' }) => {
    ...
    const handleBatchDownload = async () => {
        ...
        try {
            ...
            const errorMsg = err.response?.data?.error || err.message || '下载失败';
            alert(errorMsg);
        }
    };
    ...
    const handleBatchShare = async () => {
        ...
        try {
            const response = await axios.post('/api/share-collection',
                {
                    paths: selectedPaths,
                    name: batchShareName || `分享 - ${new Date().toLocaleDateString()}`,
                    ...
                    <div className="modal-overlay" onClick={() => setShowShareDialog(false)}>
                        <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '500px' }}>
                            ...
                            <div style={{ marginBottom: '24px' }}>
                                <label style={{ fontWeight: 600, marginBottom: '12px', display: 'block', color: 'var(--text-main)' }}>有效期:</label>
                    ...
                    <div className="preview-overlay" onClick={closePreview}>
                        ...
                        <div className="preview-actions">
                            <div className="hint" style={{ color: 'white', opacity: 0.6 }}>
                                {formatSize(previewFile.size)} • 上传者: {previewFile.uploader || 'unknown'} • 总访问: {previewFile.accessCount || 0}
                            </div>
                        </div>
                    </div>
                )
                ...
                showBatchShareDialog && (
                    <div className="modal-overlay" onClick={() => setShowBatchShareDialog(false)}>
                        <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '500px' }}>
                            ...
                            <div style={{ marginBottom: '16px' }}>
                                <div style={{ fontWeight: 600, marginBottom: '8px', color: 'var(--text-main)' }}>已选择 {selectedPaths.length} 个项目</div>
                            </div>

                            <div style={{ marginBottom: '16px' }}>
                                <label style={{ fontWeight: 600, marginBottom: '8px', display: 'block', color: 'var(--text-main)' }}>
                                    分享名称 <span style={{ fontWeight: 400, color: 'var(--text-secondary)' }}>(可选)</span>
                                </label>
                                <input
                                    type="text"
                                    value={batchShareName}
                                    onChange={(e) => setBatchShareName(e.target.value)}
                                    placeholder={`分享 - ${new Date().toLocaleDateString()}`}
                                    style={{
                                        width: '100%',
                                        padding: '12px',
                                        border: '1px solid var(--glass-border)',
