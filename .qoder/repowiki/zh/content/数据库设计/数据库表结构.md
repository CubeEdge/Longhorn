...
## 更新摘要
**所做更改**
- 新增产品状态字段迁移，引入新的状态字段（ACTIVE、IN_REPAIR、STOLEN、SCRAPPED）以跟踪产品生命周期状态
- 更新产品表结构以支持安装基线（Installed Base）功能，包括保修状态、销售溯源、所有权追踪等
- 新增搜索同义词表结构与同义词搜索功能
- 更新文件统计表列名标准化：uploader_id→uploaded_by，access_count→accessed_count，last_access→last_accessed
- 新增数据库恢复工具 restore_db.js 的功能说明
- 更新相关修复脚本中的列名使用说明
- 完善表结构文档以反映最新的列名约定、产品状态管理和新增的同义词搜索功能
...
## 简介
本文件系统性梳理 Longhorn 的数据库表结构与设计意图，聚焦核心业务表：departments（部门）、users（用户）、permissions（权限）、stars（星标，兼容旧名 starred_files）、vocabulary（词库）、search_synonyms（搜索同义词）、products（产品）。同时补充与之配套的迁移脚本、索引策略、外键约束与参照完整性，并给出表间关系图与字段说明，帮助开发者与运维人员快速理解与维护数据库。

**更新** 本次更新反映了数据库列名标准化的变更，file_stats 表现已从传统的 uploader_id、access_count、last_access 列名标准化为 uploaded_by、accessed_count、last_accessed，以提高命名一致性与可读性。新增的产品状态字段迁移引入了新的状态枚举值，支持产品生命周期的完整跟踪。新增的 search_synonyms 表为知识库搜索提供了智能化的同义词扩展功能。
...
T["server/scripts/backfill-stats.js<br/>file_stats 表创建"] --> U["file_stats 表<br/>列名标准化：uploader_id→uploaded_by"]
V["server/restore_db.js<br/>数据库恢复工具"] --> W["跨版本数据库恢复"]
X["server/service/routes/products.js<br/>产品API"] --> N
Y["server/service/routes/products-admin.js<br/>产品管理API"] --> N
Z["server/service/routes/synonyms.js<br/>同义词路由"] --> G
AA["client/src/components/ProductManagement.tsx<br/>产品管理界面"] --> N
AB["client/src/components/Knowledge/SynonymManager.tsx<br/>前端管理界面"] --> G
```
...
- [server/migrations/015_extend_products_installed_base.sql](file://server/migrations/015_extend_products_installed_base.sql#L1-L54)
- [server/migrations/017_add_product_status.sql](file://server/migrations/017_add_product_status.sql#L1-L17)
- [server/service/migrations/018_search_synonyms.sql](file://server/service/migrations/018_search_synonyms.sql#L1-L66)
- [server/scripts/backfill-stats.js](file://server/scripts/backfill-stats.js#L10-L20)
- [server/restore_db.js](file://server/restore_db.js#L1-L105)
- [server/service/routes/products.js](file://server/service/routes/products.js#L1-L288)
...
- products（产品）
  ...
  - 主键：id（自增整型）
  - 唯一约束：serial_number
  - 字段概览：id、model_name、serial_number、product_sku、product_type、status、firmware_version、production_date、description、sales_channel、original_order_id、sold_to_dealer_id、ship_to_dealer_date、current_owner_id、registration_date、sales_invoice_date、sales_invoice_proof、warranty_source、warranty_start_date、warranty_months、warranty_end_date、warranty_status、is_iot_device、is_activated、activation_date、last_connected_at、ip_address、created_at、updated_at
  - 约束：status 字段包含检查约束，仅允许 ACTIVE、IN_REPAIR、STOLEN、SCRAPPED 四个枚举值
  - 外键：sold_to_dealer_id → accounts(id)、current_owner_id → accounts(id)
  - 索引：serial_number、current_owner_id、sold_to_dealer_id、warranty_status、status

**章节来源**
- [server/index.js](file://server/index.js#L34-L77)
- [server/migrations/015_extend_products_installed_base.sql](file://server/migrations/015_extend_products_installed_base.sql#L1-L54)
- [server/migrations/017_add_product_status.sql](file://server/migrations/017_add_product_status.sql#L1-L17)
- [server/service/migrations/018_search_synonyms.sql](file://server/service/migrations/018_search_synonyms.sql#L5-L12)
- [server/seeds/vocabulary_seed.json](file://server/seeds/vocabulary_seed.json#L1-L20)
- [server/seeds/init_vocab.js](file://server/seeds/init_vocab.js#L13-L27)

## 架构总览
下图展示核心表之间的关系与关键索引策略，包括新增的产品状态表及其与知识库搜索的关系。
...
**章节来源**
- [server/index.js](file://server/index.js#L34-L38)
...
**章节来源**
- [server/index.js](file://server/index.js#L39-L48)
...
**章节来源**
- [server/index.js](file://server/index.js#L49-L57)
...
**章节来源**
- [server/index.js](file://server/index.js#L58-L64)
...
**章节来源**
- [server/index.js](file://server/index.js#L65-L77)
- [server/seeds/vocabulary_seed.json](file://server/seeds/vocabulary_seed.json#L1-L20)
- [server/seeds/init_vocab.js](file://server/seeds/init_vocab.js#L13-L27)
...
**章节来源**
- [server/service/migrations/018_search_synonyms.sql](file://server/service/migrations/018_search_synonyms.sql#L5-L12)
- [server/service/migrations/018_search_synonyms.sql](file://server/service/migrations/018_search_synonyms.sql#L14-L66)
...
**章节来源**
- [server/migrations/phase2.sql](file://server/migrations/phase2.sql#L4-L11)
- [server/migrations/phase2.sql](file://server/migrations/phase2.sql#L27-L30)
...
**章节来源**
- [server/migrations/phase2.sql](file://server/migrations/phase2.sql#L14-L25)
- [server/migrations/phase2.sql](file://server/migrations/phase2.sql#L27-L30)
...
**章节来源**
- [server/migrations/add_share_collections.sql](file://server/migrations/add_share_collections.sql#L5-L16)
...
**章节来源**
- [server/migrations/add_share_collections.sql](file://server/migrations/add_share_collections.sql#L22-L29)
- [server/migrations/add_share_collections.sql](file://server/migrations/add_share_collections.sql#L31-L31)
...
**章节来源**
- [server/scripts/backfill-stats.js](file://server/scripts/backfill-stats.js#L10-L19)
- [server/index.js](file://server/index.js#L1203-L1206)
- [server/index.js](file://server/index.js#L1364-L1367)
- [server/index.js](file://server/index.js#L3229-L3232)
...
**章节来源**
- [server/index.js](file://server/index.js#L1745-L1747)
...
- 查询与索引
  ...
  - **新增** products：serial_number、current_owner_id、sold_to_dealer_id、warranty_status、status 等字段的索引支持产品查询与状态管理
...
- 索引策略
  ...
  - file_stats：path 作为主键，访问效率高；建议在 uploaded_by 上添加索引以支持按上传者查询
  - access_logs：path 和 user_id 上的索引支持常见的查询模式
  - **新增** products：serial_number、current_owner_id、sold_to_dealer_id、warranty_status、status 等索引支持产品查询与状态管理
...
- [server/seeds/init_vocab.js](file://server/seeds/init_vocab.js#L33-L57)
- [server/restore_db.js](file://server/restore_db.js#L72-L88)
- [server/migrations/017_add_product_status.sql](file://server/migrations/017_add_product_status.sql#L6-L7)
- [server/migrations/015_extend_products_installed_base.sql](file://server/migrations/015_extend_products_installed_base.sql#L44-L45)
- [server/service/migrations/018_search_synonyms.sql](file://server/service/migrations/018_search_synonyms.sql#L14-L66)
- [server/service/routes/synonyms.js](file://server/service/routes/synonyms.js#L198-L218)

## 结论
Longhorn 的数据库围绕"部门-用户-权限-文件"主线构建，核心表结构清晰、约束明确，迁移脚本覆盖了星标、分享与批量分享集合等扩展能力。新增的产品状态字段迁移引入了完整的状态管理机制，支持产品生命周期的精确跟踪，包括 ACTIVE、IN_REPAIR、STOLEN、SCRAPPED 四种状态。新增的 search_synonyms 表为知识库搜索提供了智能化的同义词扩展功能，通过进程内缓存实现了高效的搜索召回。

**更新** 最新版本中，file_stats 表已实现列名标准化，提升了数据库设计的一致性和可读性。新增的产品安装基线功能提供了完整的销售溯源、所有权追踪和保修管理能力。新增的数据库恢复工具进一步增强了系统的可维护性，能够处理复杂的跨版本数据迁移场景。search_synonyms 表的引入标志着系统在智能化搜索方面的重大进步，为用户提供更加精准和全面的知识检索体验。

建议在高并发或复杂权限查询场景下，根据实际业务行为进一步评估与补充索引策略，特别是在 file_stats 表的 uploaded_by 字段上考虑添加适当的索引以优化按上传者查询的性能。同时，products 表的状态字段检查约束应定期验证其有效性，确保产品状态管理的准确性。search_synonyms 表的缓存机制应定期验证其有效性，确保搜索功能的稳定运行。