...
- [server/migrations/039_add_product_metadata_fields.sql](file://server/migrations/039_add_product_metadata_fields.sql)
...
- [server/service/routes/parts-master.js](file://server/service/routes/parts-master.js)
...
## 更新摘要
**所做更改**
- 新增sku_prices通用价格表，支持产品、配件、附件的统一价格管理
- 新增material_id字段，统一物料ID/ SaaS ID管理
- 新增产品目录设置功能，支持产品族可见性控制和产品类型过滤
- 新增产品物理属性字段，包括重量、体积、尺寸等元数据
- 新增危险品标识、UPC条形码、序列号前缀等SKU元数据
- 新增产品模型物料ID字段，支持品牌和内部前缀管理
- 更新产品架构升级，完善三级产品管理体系
- 新增配件系统价格剥离迁移，实现价格数据的规范化管理
...
## 简介
本文件系统性梳理 Longhorn 的数据库表结构与设计意图，聚焦核心业务表：departments（部门）、users（用户）、permissions（权限）、stars（星标，兼容旧名 starred_files）、vocabulary（词库）、search_synonyms（搜索同义词）、products（产品）、product_models（产品模型）、product_skus（产品SKU）、sku_prices（通用价格表）。同时补充与之配套的迁移脚本、索引策略、外键约束与参照完整性，并给出表间关系图与字段说明，帮助开发者与运维人员快速理解与维护数据库。

**更新** 本次更新反映了数据库架构的重大现代化改进，新增了完整的三级产品管理体系和统一的价格管理机制。sku_prices表实现了产品、配件、附件的统一价格管理，支持多币种定价和成本核算。material_id字段统一了物料ID和SaaS ID的管理，提升了数据一致性。产品目录设置功能支持灵活的产品族可见性控制和产品类型过滤，增强了用户体验。产品物理属性、危险品标识、UPC条形码、序列号前缀等元数据字段完善了SKU的精细化管理能力。
...
- **新增** 通用价格表sku_prices通过迁移脚本实现价格数据的规范化管理
- **新增** 产品目录设置功能通过系统设置表实现产品展示的灵活控制
...
JJ["server/migrations/upgrade_parts_pricing.js<br/>新增：通用价格表迁移"] --> KK["sku_prices 表<br/>新增：统一价格管理"]
LL["server/migrations/047_add_product_dropdown_settings.sql<br/>新增：产品目录设置"] --> MM["system_settings 表<br/>新增：产品族可见性控制"]
```
...
- [server/migrations/upgrade_parts_pricing.js:1-92](file://server/migrations/upgrade_parts_pricing.js#L1-L92)
...
- product_models（产品模型）
  ...
  - 主键：id（自增整型）
  - 唯一约束：model_name
  - 字段概览：id、model_name、internal_name、product_family、product_type、description、is_active、name_en、brand、internal_prefix、hero_image、material_id、created_at、updated_at
  - 外键：无
...
- **新增** sku_prices（通用价格表）
  - 设计目的：统一管理产品、配件、附件的价格数据，支持多币种定价
  - 主键：id（自增整型）
  - 唯一约束：sku
  - 字段概览：id、sku、item_type、price_cny、price_usd、price_eur、cost_cny、updated_at
  - 约束：item_type 仅允许 'part'、'product'、'accessory' 三值之一
  - 索引：sku（唯一）

- **新增** system_settings（系统设置）
  - 设计目的：控制产品目录的显示设置，包括产品族可见性和产品类型过滤
  - 主键：id（自增整型）
  - 字段概览：id、show_family_a、show_family_b、show_family_c、show_family_d、show_family_e、enable_product_type_filter、allowed_product_types、updated_at
  - 默认值：各产品族显示开关默认值，产品类型过滤默认开启
...
- [server/migrations/upgrade_parts_pricing.js:22-35](file://server/migrations/upgrade_parts_pricing.js#L22-L35)
- [server/migrations/047_add_product_dropdown_settings.sql:6-25](file://server/migrations/047_add_product_dropdown_settings.sql#L6-L25)

## 架构总览
下图展示核心表之间的关系与关键索引策略，包括新增的产品模型、产品SKU、通用价格表和产品目录设置。
...
int show_family_d
int show_family_e
int enable_product_type_filter
...
- [server/service/migrations/034_fix_product_models_and_seed.sql:33-48](file://server/service/migrations/034_fix_product_models_and_seed.sql#L33-L48)
- [server/migrations/039_add_product_metadata_fields.sql:7](file://server/migrations/039_add_product_metadata_fields.sql#L7)
...
### **新增** sku_prices（通用价格表）
- 设计要点
  - 统一价格管理：支持产品、配件、附件的统一价格数据管理
  - 多币种支持：支持人民币、美元、欧元三种货币定价
  - 成本核算：支持成本价的单独管理
  - 类型区分：通过 item_type 字段区分不同类型的商品
  - 唯一SKU约束：确保每个SKU只有一套价格数据
- 字段说明
  - id：主键，自增整型
  - sku：唯一文本，商品SKU代码
  - item_type：文本，商品类型，仅允许 'part'、'product'、'accessory'
  - price_cny：十进制，人民币单价，默认 0
  - price_usd：十进制，美元单价，默认 0
  - price_eur：十进制，欧元单价，默认 0
  - cost_cny：十进制，成本价，默认 0
  - updated_at：时间戳，默认当前时间
- 约束与索引
  - 唯一约束：sku
  - 约束：item_type 仅允许 'part'、'product'、'accessory'
  - 索引：sku（唯一）
...
### **新增** system_settings（系统设置）
- 设计要点
  - 产品目录控制：控制不同产品族在目录中的显示
  - 产品类型过滤：支持产品类型的筛选和过滤
  - 默认配置：提供合理的默认显示设置
  - 灵活配置：支持管理员动态调整显示策略
- 字段说明
  - id：主键，自增整型
  - show_family_a：整数，控制产品族A的显示，默认 1（显示）
  - show_family_b：整数，控制产品族B的显示，默认 0（隐藏）
  - show_family_c：整数，控制产品族C的显示，默认 1（显示）
  - show_family_d：整数，控制产品族D的显示，默认 1（显示）
  - show_family_e：整数，控制产品族E的显示，默认 0（隐藏）
  - enable_product_type_filter：整数，启用产品类型过滤，默认 1（启用）
  - allowed_product_types：文本，允许的产品类型列表，默认包含多种类型
  - updated_at：时间戳，默认当前时间
- 约束与索引
  - 无特殊约束
  - 无额外索引
...
- **新增** **价格管理相关表**：sku_prices（支持统一价格管理）
...
- 表间依赖
  ...
  - **新增** sku_prices }o--|| parts_master（外键：sku）
  - **新增** system_settings }o--|| products（通过产品目录设置控制显示）
- 查询与索引
  ...
  - **新增** product_models：product_family、is_active、material_id 等字段的索引支持产品模型查询
  - **新增** product_skus：model_id、is_active、material_id 等字段的索引支持SKU查询
  - **新增** sku_prices：sku（唯一）索引支持快速价格查询
  - **新增** search_synonyms：通过进程内 Map 缓存提供高效查询，无需数据库索引
  - **新增** system_settings：无特殊索引需求，通过默认配置提供系统设置
...
- 索引策略
  ...
  - **新增** product_models：product_family、is_active、material_id 等索引支持产品模型查询
  - **新增** product_skus：model_id、is_active、material_id 等索引支持SKU查询
  - **新增** sku_prices：sku（唯一）索引支持快速价格查询
...
- 批量操作
  ...
  - **新增** sku_prices 价格数据镜像同步，通过单事务保证数据一致性
- 主键选择
  ...
  - **新增** sku_prices 使用 id 作为主键，sku 字段的唯一约束确保价格数据唯一性
  - **新增** search_synonyms 使用自增主键，配合进程内缓存实现高效查询
  - **新增** system_settings 使用 id 作为主键，提供系统级别的配置管理
...
- **新增** 通用价格表初始化失败
  - 现象：sku_prices 表创建或数据同步失败
  - 排查：检查 upgrade_parts_pricing.js 脚本执行日志，确认 parts_master 表存在且包含价格数据
- **新增** 产品目录设置异常
  - 现象：产品目录显示不符合预期
  - 排查：检查 system_settings 表中的显示配置，确认产品族可见性设置正确
- **新增** 物料ID管理问题
  - 现象：产品模型或SKU的物料ID显示异常
  - 排查：确认 material_id 字段是否正确更新，检查产品模型表的字段重命名迁移是否完成
...
- [server/migrations/upgrade_parts_pricing.js:15-92](file://server/migrations/upgrade_parts_pricing.js#L15-L92)
- [server/migrations/047_add_product_dropdown_settings.sql:16-25](file://server/migrations/047_add_product_dropdown_settings.sql#L16-L25)
...
**新增** 本次更新最重要的成果是实现了数据库架构的现代化升级。sku_prices通用价格表的引入彻底解决了价格数据分散的问题，支持产品、配件、附件的统一价格管理，为未来的商业扩展奠定了坚实基础。material_id字段的统一管理提升了数据的一致性和可维护性。产品目录设置功能的加入使得产品展示更加灵活可控，能够根据不同业务场景调整产品族的可见性。这些改进不仅提升了系统的功能性，更重要的是为Longhorn的长期发展提供了强大的技术支撑。

建议在高并发或复杂权限查询场景下，根据实际业务行为进一步评估与补充索引策略，特别是在 file_stats 表的 uploaded_by 字段上考虑添加适当的索引以优化按上传者查询的性能。同时，products、product_models、product_skus、sku_prices 表的状态字段检查约束应定期验证其有效性，确保产品管理体系的准确性。search_synonyms 表的缓存机制应定期验证其有效性，确保搜索功能的稳定运行。产品SKU与产品模型的关联关系应定期检查，确保产品数据的完整性和一致性。sku_prices表的价格数据应定期审计，确保价格信息的准确性和时效性。system_settings表的配置应根据业务变化及时调整，确保产品展示符合最新的业务需求。