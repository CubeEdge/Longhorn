...
## 更新摘要
**变更内容**
- 新增三层次产品架构，引入产品目录、商品规格和设备台账的完整数据模型
- 新增产品模型表（product_models）支持品牌、型号代码和主视觉图片管理
- 新增产品SKU表（product_skus）支持销售形态和ERP编码管理
- 新增设备台账表（products）支持序列号、品质等级和仓库位置管理
- 新增产品状态字段（status）支持设备生命周期管理
- 新增产品架构升级迁移脚本，实现三层架构的完整数据结构
- 新增产品管理相关API接口，支持三层架构的完整CRUD操作
- 新增产品管理前端组件，支持三层架构的可视化管理
...
**重大更新** 系统已完成三层次产品架构的重大升级，引入了产品目录（Model）、商品规格（SKU）和设备台账（Instance）的完整三层架构，实现了与内部ERP系统的深度对齐。新架构解决了"相同型号多种配置"和"硬件品质等级"的管理难题，支持物料代码（9系列）和商品SKU（A系列）的统一管理。

**新增产品架构功能** 三层产品架构包含：
- **产品目录（Model）**：定义基础平台硬件型号，支持品牌、型号代码和主视觉图片管理
- **商品规格（SKU）**：定义具体的销售形态，支持ERP编码和全家福图片管理
- **设备台账（Instance）**：定义每一台出厂的具体设备，支持序列号、品质等级和仓库位置管理

**新增产品管理API** 支持三层架构的完整CRUD操作：
- 产品模型管理API：支持产品目录的创建、更新、查询和删除
- 商品规格管理API：支持SKU的创建、更新、查询和删除
- 设备台账管理API：支持设备实例的创建、更新、查询和删除

**新增产品管理前端组件** 提供三层架构的可视化管理：
- 产品模型管理界面：支持产品目录的双页签管理（基本信息、SKU体系）
- 商品规格管理界面：支持SKU的列表查看和编辑
- 设备台账管理界面：支持设备实例的列表查看和编辑
...
├── due_date: DATE -- 到期日
├── bill_to_name: TEXT -- 收票方名称
├── bill_to_address: TEXT -- 收票方地址
├── bill_to_country: TEXT -- 收票方国家
├── subtotal: REAL DEFAULT 0 -- 小计
├── shipping_cost: REAL DEFAULT 0 -- 运费
...
├── total_amount: REAL DEFAULT 0 -- 总金额
├── currency: TEXT DEFAULT 'USD' -- 货币
├── exchange_rate: REAL DEFAULT 1 -- 汇率
├── payment_terms: TEXT DEFAULT 'Net30' -- 付款条件
├── payment_status: TEXT DEFAULT 'Pending' -- 付款状态
├── paid_amount: REAL DEFAULT 0 -- 已付款
├── paid_date: DATE -- 付款日期
├── bank_details: TEXT -- 银行信息(JSON)
...
- INDEX idx_pi_dealer (dealer_id)
- INDEX idx_pi_status (status)
- INDEX idx_pi_payment (payment_status)
...
PRODUCT_MODELS ||--o{ PRODUCT_SKUS : "has"
PRODUCT_SKUS ||--o{ PRODUCTS : "defines"
PRODUCTS ||--o{ INQUIRY_TICKETS : "relates_to"
PRODUCTS ||--o{ TICKETS : "affects"
PRODUCT_MODELS ||--o{ INQUIRY_TICKETS : "relates_to"
PRODUCT_MODELS ||--o{ TICKETS : "affects"
ACCOUNTS ||--o{ CONTACTS : "has"
ACCOUNTS ||--o{ ACCOUNT_DEVICES : "owns"
ACCOUNTS ||--o{ INQUIRY_TICKETS : "creates"
ACCOUNTS ||--o{ TICKETS : "creates"
ACCOUNTS ||--o{ DEALER_INVENTORY : "manages"
ACCOUNTS ||--o{ RESTOCK_ORDERS : "places"
ACCOUNTS ||--o{ PROFORMA_INVOICES : "receives"
ACCOUNTS ||--o{ ACCOUNT_TYPE_HISTORY : "converted_from"
ACCOUNTS ||--o{ ACCOUNT_TRANSFERS : "transferred_to"
CONTACTS ||--o{ INQUIRY_TICKETS : "creates"
CONTACTS ||--o{ TICKETS : "creates"
CONTACTS ||--o{ ACCOUNT_TRANSFERS : "transferred_from"
ACCOUNT_DEVICES ||--o{ INQUIRY_TICKETS : "relates_to"
ACCOUNT_DEVICES ||--o{ TICKETS : "relates_to"
...
// 请求体示例
{
  ...
  "due_date": "2026-02-28",
  "bill_to_name": "深圳经销商有限公司",
  "bill_to_address": "深圳市南山区科技园",
  "bill_to_country": "中国",
  "subtotal": 1500.00,
  "shipping_cost": 50.00,
  "tax_amount": 0,
...
7. **文件权限索引**：新增 path_hash 和 expires_at 索引，优化权限查询性能
...
**三层产品架构优化**：
- 使用 product_models、product_skus、products 三层结构支持高效查询
- 通过外键约束确保数据完整性
- 通过索引优化三层结构的关联查询性能
- 支持产品模型、SKU和设备的独立管理和查询
- 提供三层结构的统计和分析功能

**产品模型管理优化**：
- 使用 name_zh 和 model_code 索引优化产品模型查询
- 支持品牌、型号代码和主视觉图片的快速检索
- 提供产品族群和产品类型的过滤查询
- 支持产品模型的启用/禁用状态管理

**商品规格管理优化**：
- 使用 sku_code 和 model_id 索引优化SKU查询
- 支持ERP编码和显示名称的快速检索
- 提供规格标签和SKU图片的管理功能
- 支持SKU的启用/禁用状态管理

**设备台账管理优化**：
- 使用 serial_number 和 sku_id 索引优化设备查询
- 支持序列号、品质等级和仓库位置的快速检索
- 提供设备状态和品质等级的过滤查询
- 支持设备的生命周期管理和状态跟踪

**产品状态管理优化**：
- 使用 status 索引优化设备状态查询
- 支持ACTIVE/IN_REPAIR/STOLEN/SCRAPPED状态的快速过滤
- 提供设备状态的统计和分析功能
- 支持设备状态的自动更新和维护

**产品SKU关联优化**：
- 使用外键约束确保SKU与设备的关联完整性
- 支持SKU与设备的双向关联查询
- 提供SKU使用情况的统计和分析功能
- 支持SKU的启用/禁用状态管理

**仓库位置追踪优化**：
- 使用 warehouse 和 entry_channel 索引优化仓库查询
- 支持设备仓库位置的实时追踪
- 提供仓库位置的统计和分析功能
- 支持入库渠道的分类管理
