...
## 更新摘要
**变更内容**
- 新增文件权限管理功能章节，详细介绍 file_permissions 表结构和权限查询优化
- 更新权限验证与并发控制章节，增加 path_hash 字段和索引优化
- 新增权限中间件和权限检查器的实现细节
- 更新 API 接口文档，增加权限管理相关接口
- 新增权限迁移脚本和数据模型说明
...
**更新** 新增完善的文件权限管理体系，包括 file_permissions 表、path_hash 字段优化和权限查询中间件。
...
- [server/migrate_file_permissions.js](file://server/migrate_file_permissions.js#L1-L139)
...
## 架构总览
系统采用"前端组件 + 服务端 API"的分层设计，移动端通过服务层封装网络调用，缓存与预取策略贯穿前后端，确保流畅的交互体验。**新增** 权限中间件在服务端提供统一的权限控制机制。
...
```mermaid
flowchart TD
Start(["进入目录"]) --> Fetch["拉取文件列表<br/>SWR/iOS缓存"]
...
**图表来源**
- [ios/LonghornApp/Models/FileItem.swift](file://ios/LonghornApp/Models/FileItem.swift#L1-L200)
...
**图表来源**
- [client/src/components/FileBrowser.tsx](file://client/src/components/FileBrowser.tsx#L770-L800)
- [server/index.js](file://server/index.js#L1-L200)
- [ios/LonghornApp/Services/PreviewCacheManager.swift](file://ios/LonghornApp/Services/PreviewCacheManager.swift#L1-L120)
...
**章节来源**
- [server/index.js](file://server/index.js#L120-L129)
- [server/index.js](file://server/index.js#L1570-L1573)
- [server/index.js](file://server/index.js#L1672-L1694)
- [server/migrate_file_permissions.js](file://server/migrate_file_permissions.js#L1-L139)
- [server/service/middleware/permission.js](file://server/service/middleware/permission.js#L1-L278)
- [docs/Service_DataModel.md](file://docs/Service_DataModel.md#L1470-L1518)
...
**章节来源**
- [client/src/components/FileBrowser.tsx](file://client/src/components/FileBrowser.tsx#L308-L340)
- [ios/LonghornApp/Views/Files/FileBrowserView.swift](file://ios/LonghornApp/Views/Files/FileBrowserView.swift#L308-L344)
- [ios/LonghornApp/Services/FileService.swift](file://ios/LonghornApp/Services/FileService.swift#L78-L110)
...
**章节来源**
- [client/src/components/FileBrowser.tsx](file://client/src/components/FileBrowser.tsx#L248-L271)
- [ios/LonghornApp/Services/FileService.swift](file://ios/LonghornApp/Services/FileService.swift#L227-L246)
...
**章节来源**
- [server/index.js](file://server/index.js#L1540-L1589)
- [server/index.js](file://server/index.js#L1671-L1694)
...
**章节来源**
- [client/src/components/FileBrowser.tsx](file://client/src/components/FileBrowser.tsx#L72-L102)
- [client/src/hooks/useCachedFiles.ts](file://client/src/hooks/useCachedFiles.ts#L1-L102)
- [client/src/store/useAuthStore.ts](file://client/src/store/useAuthStore.ts#L1-L31)
...
- [docs/Service_DataModel.md](file://docs/Service_DataModel.md#L1498-L1507)
...
**更新** 权限查询性能优化显著提升系统响应速度，特别是大量文件夹权限检查场景。
...
## 结论
本系统通过前后端协同的缓存与并发策略，实现了跨端一致的文件管理体验。上传采用分片合并，下载与预览具备良好的性能与稳定性；**新增的权限管理体系**提供了完善的文件访问控制，包括基于角色的权限控制、path_hash 查询优化和 View As 功能；统计与批量操作满足企业协作需求。建议持续优化缓存淘汰策略与错误恢复机制，进一步提升大规模场景下的稳定性。
...
**章节来源**
- [client/README.md](file://client/README.md#L18-L31)
