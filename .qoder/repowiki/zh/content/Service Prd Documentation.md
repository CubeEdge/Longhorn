...
## 更新摘要
**变更内容**
- 更新配件兼容性SSOT重构章节，反映从parts_master.compatible_models冗余字段到product_model_parts中间表的架构变更
- 新增配件兼容性搜索逻辑和响应格式说明
- 更新配件管理API文档和前端组件实现细节
- 增强BOM（物料清单）管理和产品型号关联机制
...
- **按需穿透 (Just-in-Time Access)**：OP/RD 只有在成为工单的 Assignee 或 Participant 时，才获得该工单及关联资产的只读权限
...
**重要更新**：配件兼容性系统已完成SSOT（单一真实来源）重构，从冗余的 `parts_master.compatible_models` 字段迁移到以 `product_model_parts` 中间表为核心的数据模型。
...
PARTS_MASTER ||--o{ PRODUCT_MODEL_PARTS : "has_many"
PRODUCT_MODELS ||--o{ PRODUCT_MODEL_PARTS : "has_many"
```
...
**前端搜索优化**：
- 优先使用 `product_model_id` 参数进行精确查询
- 当提供产品型号ID时，系统通过 `product_models` 表获取型号信息，然后匹配 `parts_master.compatible_models` JSON 字段
- 支持中文名称、英文名称和型号代码的模糊匹配

**后端查询优化**：
- 使用 `json_group_array` 动态生成 `compatible_models` 虚字段
- 通过 `product_model_parts` 中间表维护多对多关系
- 支持 BOM（物料清单）推荐和兼容性查询
  ...
  "data": [
    {
      ...
      "name": "SDI模块",
      "category": "接口",
      "compatible_models": ["ME8K", "ME6K"], // 基于关联表动态生成的虚字段
...
- [parts-master.js:165-226](file://server/service/routes/parts-master.js#L165-L226)
- [parts-master.js:494-608](file://server/service/routes/parts-master.js#L494-L608)
- [Service_API.md:15-17](file://docs/Service_API.md#L15-L17)
- [Service_API.md:4490-4532](file://docs/Service_API.md#L4490-L4532)
...
- 根据产品型号获取BOM推荐配件
- 优先查询 `product_model_parts` 表的常用配件
- 当BOM表为空时，回退到 `compatible_models` 字段的模糊匹配
- 支持优先级排序和常用性标记
...
**章节来源**
- [compatibility.js:16-90](file://server/service/routes/compatibility.js#L16-L90)
- [compatibility.js:268-317](file://server/service/routes/compatibility.js#L268-L317)
...
**新增性能优化措施**：
- **中间表索引**：为 `product_model_parts` 表建立复合索引
- **JSON字段优化**：对 `compatible_models` JSON字段进行全文检索优化
- **查询缓存**：热门产品型号的兼容性查询结果缓存
- **分页优化**：配件查询支持更大数据集的分页处理

**章节来源**
- [parts-master.js:66-83](file://server/service/routes/parts-master.js#L66-L83)
- [031_parts_master.sql:195-212](file://server/service/migrations/031_parts_master.sql#L195-L212)
...
**配件兼容性查询失败**
- 验证 `product_model_id` 参数有效性
- 检查 `product_model_parts` 中间表数据完整性
- 确认 `compatible_models` JSON字段格式正确
...
**GET /api/v1/parts-master**：
- 支持 `product_model_id` 参数进行精确兼容性查询
- 返回 `compatible_models` 虚字段和 `model_associations` 原始数据
- 支持分页和条件过滤

**POST /api/v1/parts-master**：
- `compatible_models` 参数接受产品型号ID数组
- 系统自动维护 `product_model_parts` 中间表
...
- [Service_API.md:4490-4532](file://docs/Service_API.md#L4490-L4532)
- [parts-master.js:28-159](file://server/service/routes/parts-master.js#L28-L159)
- [parts-master.js:309-323](file://server/service/routes/parts-master.js#L309-L323)
- [parts-master.js:414-435](file://server/service/routes/parts-master.js#L414-L435)