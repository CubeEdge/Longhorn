...
(async () => {
        ...
        const authenticate = (req, res, next) => {
            ...
            if (authHeader) {
                ...
                jwt.verify(token, JWT_SECRET, (err, decoded) => {
                    ...
                    let user;
                    try {
                        // Simplified query to avoid SQLITE_ERROR
                        user = db.prepare(`
                    SELECT u.id, u.username, u.role, u.user_type, u.dealer_id, u.department_id, d.name as department_name 
                    FROM users u
                    LEFT JOIN departments d ON u.department_id = d.id
                    WHERE u.id = ?
                `).get(decoded.id);

                        // Backfill user_type if missing (migration logic)
                        if (user && !user.user_type) {
                            user.user_type = user.role === 'Dealer' ? 'Dealer' : 'Internal';
                        }
                    } catch (authErr) {
                        console.error('[AUTH DB ERROR]', authErr.message, 'Decoded:', decoded);
    ...
    `).get(username);
        ...
        // ==========================================
        // File Management Module (Internal Only)
        // ==========================================
        app.use('/api', filesRoutes(db, authenticate, DISK_A, THUMB_DIR, RECYCLE_DIR, upload, chunkUpload));
        ...
        // --- Users CRUD (Restored & Enhanced) ---

        app.post('/api/admin/users', authenticate, isAdmin, (req, res) => {
            const { username, password, role, department_id, user_type, dealer_id } = req.body;
            const hash = bcrypt.hashSync(password, 10);
            try {
                // Modified INSERT to include user_type and dealer_id
                const result = db.prepare('INSERT INTO users (username, password, role, department_id, user_type, dealer_id) VALUES (?, ?, ?, ?, ?, ?)').run(username, hash, role || 'Member', department_id, user_type || 'Internal', dealer_id || null);
        ...
        app.get('/api/admin/users', authenticate, (req, res) => {
            ...
            let query = `
                SELECT u.id, u.username, u.role, u.user_type, u.dealer_id, u.department_id, u.created_at, d.name as department_name 
                FROM users u 
                LEFT JOIN departments d ON u.department_id = d.id
            `;
            let params = [];
        ...
        app.put('/api/admin/users/:id', authenticate, (req, res) => {
            ...
            try {
                ...
                if (dealer_id !== undefined) { updates.push("dealer_id = ?"); params.push(dealer_id); }
                if (password) {
        ...
        // ==========================================
        // Customer Archive API (Section 2.3)
        // ==========================================
        ...
        app.get('/api/v1/customers', authenticate, (req, res) => {
            try {
                ...
                if (req.user.role === 'Dealer') {
                    // Dealers can ONLY see their own EndUsers (or customers they created)
                    sql += ` AND (parent_dealer_id = ? OR customer_type = 'EndUser' AND parent_dealer_id = ?)`;
                    params.push(req.user.dealer_id, req.user.dealer_id);

                    // Force account_type to Customer if Dealer (UI should hide tab anyway, but for API safety)
                    // Actually, account_type filter is optional. 
                    // The key is: A Dealer CANNOT see other Dealers. 
                    // A Dealer CAN see customers assigned to them.

                    // Revised logic:
                    // 1. Must be THEIR customer (parent_dealer_id = MY_ID)
                    // 2. OR implicitly themselves (if they want to see their own profile? maybe not needed now)

                    // Let's simplify: A Dealer manages their "Clients". 
                    // So just filter by parent_dealer_id.
                    // But wait, what if the dealer_id is null in DB?

                    // Let's assume req.user.dealer_id IS populated (index.js 505 confirms it's fetched).
                    // But wait, the previous `sqlite` output showed `dealer_id` column in `users` table.

                    // However, `customers` table schemas:
                    // 15|parent_dealer_id|INTEGER|0||0
                ...
                if (account_type === 'Dealer') {
                    // Query Dealers Table if specifically asked for Dealer list
                    let dealerSql = `
                        SELECT 
                            id, 
                    ...
                    if (name) {
                        dealerSql += ` AND (name LIKE ? OR contact_person LIKE ? OR contact_phone LIKE ?)`;
                        const like = `%${name}%`;
                        dealerParams.push(like, like, like);
                    }

                    dealerSql += ` ORDER BY created_at DESC LIMIT ? OFFSET ?`;
                    dealerParams.push(limit, offset);

                    const list = db.prepare(dealerSql).all(...dealerParams);

                    // Count
                    let dealerCountSql = `SELECT COUNT(*) as total FROM dealers WHERE 1=1`;
                    const dealerCountParams = [];

                    if (name) {
                        dealerCountSql += ` AND (name LIKE ? OR contact_person LIKE ? OR contact_phone LIKE ?)`;
                        const like = `%${name}%`;
                        dealerCountParams.push(like, like, like);
                    }
                ...
                // If account_type is Customer, filter accordingly.
                // If account_type is not provided, we might want to return mixed? 
                // For now, let's keep existing logic but ensure 'Dealer' type is handled above.

                if (type) { // Legacy param support (EndUser/Dealer/Distributor/Internal)
                ...
                const countParams = []; // Rebuild params for count
                ...
                if (account_type) {
                    if (account_type === 'Customer') {
                        countSql += ` AND customer_type = 'EndUser'`;
                    } else {
                        countSql += ` AND customer_type = ?`;
                        countParams.push(account_type);
                    }
                }

                // Permission Scoping for COUNT
                if (req.user.role === 'Dealer') {
                    if (!req.user.dealer_id) {
                        countSql += ` AND 1=0`;
                    } else {
                        countSql += ` AND parent_dealer_id = ?`;
                        countParams.push(req.user.dealer_id);
                    }
                }
        ...
        app.post('/api/v1/customers', authenticate, (req, res) => {
            try {
                ...
                if (req.user.role === 'Dealer') {
                    if (!req.user.dealer_id) return res.status(403).json({ error: 'Dealer account not linked to Dealer Record' });
                    // Force fields for Dealer
                    parentDealerId = req.user.dealer_id;
                    finalCustomerType = 'EndUser'; // Dealers can only create EndUsers
                }
                ...
                `).run(
                    ...
                    'EndUser', service_tier || 'STANDARD', industry_tags || null, parentDealerId
                );
    ...
    `).get(userId);
        ...
        // Serve service uploads from DiskA
        app.use('/uploads/service', express.static(SERVICE_UPLOADS_DIR, {
            maxAge: '1d',
            etag: true
        }));
