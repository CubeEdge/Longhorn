...
| :--- | :--- | :--- | :--- | :--- |
| 2026-01-11 11:30 | *文档整理与归档* | `pending` | `Done` | **文档结构重构**：创建 `product_docs/` 目录，归档 PRD/Log/Guide。更新 PRD 到 V11.3.0，新建 `deployment.md` 记录服务器运维命令。 |
| 2026-01-11 11:25 | *iPhone 药丸屏适配* | `3be5332` | `Done` | **Safe Area 适配**：修复 iPhone 14 Pro+ 顶部遮挡问题。`index.html` 添加 `viewport-fit=cover`，`index.css` 增加 `env(safe-area-inset-top)` 等边距处理。 |
| 2026-01-11 00:55 | *UI综合修复(V11.2)* | `d5e5d31` | `Done` | **UI/UX 大修**：(1) 修复授权弹窗 z-index 被遮挡；(2) 彻底屏蔽 `.chunks` 隐藏文件；(3) 恢复 Sidebar 用户名两行显示；(4) 用户菜单增加 Dashboard 入口；(5) 移动端横屏滚动修复。 |
