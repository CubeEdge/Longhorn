...
⚠️ **风险**：客服在回复客户时，容易手滑把 Internal Answer 发出去

**解决方案**：
1. **引用知识库按钮**：点击"引用知识库"时，默认**只复制 external_answer**
   - 复制按钮显示为 `[复制外部答案]`
   - 复制成功后显示提示："已复制外部答案，可直接发送给客户"

2. **内部答案访问需二次确认**：
   - 如果客服想复制内部答案，需点击专门的 `[查看内部答案]` 按钮
   - 弹出确认对话框：
     ```
     ⚠️ 警告：内部答案包含技术细节
     
     内部答案仅供内部参考，不建议直接发送给客户。
     
     [ 取消 ]  [ 我知道风险，继续查看 ]
     ```
   - 确认后才显示内部答案，并且复制按钮显示为 `[复制内部答案⚠️]`（红色警告样式）

3. **视觉区分**：
   - External Answer: 绿色边框，✓ 图标
   - Internal Answer: 黄色边框，⚠️ 图标

4. **审计日志**：
   - 记录每次"复制内部答案"的操作，防止滥用
