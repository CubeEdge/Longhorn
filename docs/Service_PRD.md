...
**版本**: 12.3.410
**状态**: 迭代中
**最后更新**: 2026-03-22 (维修报告财务纠故与价格快照锁定)

> **v12.3.410 更新 (维修报告财务纠故与价格快照锁定)**：
> - **财务纠故**: 修复了 USD 符号显示不一致（统一为 `US $`）、工时费用小计为 0 以及已删除配件无法保存的问题。
> - **快照锁定**: 增强了已发布/审批报告的价格锁定逻辑，防止单价随系统基准价变动，确保财务审计一致性。
> - **响应式优化**: 实现了财务概览面板的毫秒级实时计算，消除了子部分金额与总额显示不同步的延迟。
>
> **v12.3.358 更新 (配件消耗重构与PI德文预览)**：
> - **配件消耗底座**: 修复自动保存过程中 `parts_consumption` 远端表单 `source_type` 等约束导致的 500 异常。
> - **PI 德语支持**: 为 PI 预览界面增设 Deutsch (de-DE) 多语言切换，包含全量表单及术语文案翻译。
> - **UI 层级优化**: 清除数字类字段的硬编码等宽字体字号，应用全局统一的 `tabular-nums` 数字格式化样式。

> **v12.3.400 更新 (价格体系 SSOT 重构)**：
> - **SSOT 架构确立**: 正式移除 `parts_master` 表中的冗余价格字段（`price_cny/usd/eur`, `cost_cny`），统一由 `sku_prices` 作为唯一事实来源。
> - **字段修复**: 修复了 `parts_consumption` 表缺失 `is_deleted` 字段导致的 SQL 错误，同步补全了软删除及审计字段。
> - **架构同步**: 全面更新了数据模型文档及关联的导入/种子脚本，确保系统数据流的一致性。
>
> **v12.3.352 更新 (维修报告/PI 多语言预览)**：
> - **配件英文名称显示**: 维修报告和 PI 预览时，非中文语言下配件名称自动显示为数据库中的英文名称（通过 SKU 查询）。
> - **双语字段方案**: 工时费用和其他费用支持 `description_en` 字段，用于非中文预览时显示英文描述。
> - **PI 预览增强**: 新增其他费用显示区域，支持多语言标签。
> - **PI 导入优化**: 从维修报告导入后不再关闭窗口，仅更新数据。
> - **API 修复**: 修复 `GET /api/v1/parts-master/bom` 路由顺序导致的 404 错误。
>
> **v12.3.270 更新 (配件兼容性 SSOT 重构)**：
> - 核心重构：废弃并物理删除了 `parts_master.compatible_models` 冗余字段。
> - 架构升级：建立以 `product_model_parts` 中间表为单一人心数据源（SSOT）的配件兼容性模型。
> - 系统影响：更新了配件导入、数据初始化及前端列表/详情页的处理逻辑。
...
为避免产品生命周期和配件兼容关系被混用，服务系统在产品层面引入 5 个具有不同服务逻辑的族群，用于驱动工单创建、配件候选列表和维修报价规则。
...
**B 类：广播摄像机 (Broadcast Camera)**
- **范围**：针对广电、现场制作领域的摄像机及系统。
- **服务特性**：
  - 在工单、保修等模块独立于 Cinema 摄影机归类，以便广电业务端单独切分和统计。
  - 授权维修等模块可关联广播级专用接口箱、供电板等配件模块候选。

**C 类：电子寻像器 (Eagle e-Viewfinder)**
...
**D 类：存档/历史机型 (Archived Cine Cameras)**
- **范围**：已停产但仍提供维修服务的机型，如 MAVO LF、MAVO S35、Terra 4K/6K 等。
- **服务特性**：
  - 前端需折叠或明确标记为「历史机型」，避免干扰新用户选型，但仍支持创建工单和返修。
  - 仅关联老一代专属附件（如 KineMAG SSD (SATA)、老款手柄等），不暴露新一代专属配件。

**E 类：通用附件 (Universal Accessories)**
- **范围**：严格限定为跨代通用、与具体机型兼容性弱相关的附件，例如：
  - GripBAT 系列电池等标准供电附件；
  - Magic Arm / Dark Tower 等安装类通用附件。
- 约束：
  - E 类附件在创建工单时可以不强制绑定具体机型，但系统仍鼓励填写实际关联机型以便后续分析。
  - **KineMON 系列监视器仅保证与 Kinefinity 电影摄影机配合使用，不属于 E 类通用附件**；在第三方机型场景下，系统需提示兼容性风险，不将其视作「通用显示器」。
...
1. **存储卡隔离规则**
   ...
   - KineMAG SSD (SATA)：仅当产品属于 D 类（MAVO LF/Terra 系列）时，才作为配件候选出现；
...
#### 1.5.6 三层产品管理体系 (Product Hierarchy Management)

为应对复杂的销售套装（Bundles）与实物资产管理需求，系统实现三层产品管理模型：

1. **产品型号 (Product Model)**:
   - 内部研发视角。如 "MAVO Edge 8K"。
   - 包含品牌、产品族群（A/B/C/D）、型号短码。
   - 管理该型号下的所有 SKU 和实物资产。

2. **产品规格 (Product SKU)**:
   - 商业/销售视角。如 "MAVO Edge 8K 基础套装 (Basic Kit)"。
   - 包含 A 系列 SKU 编码、规格详情（如颜色、卡口）、套装封面图。
   - 决定实物出厂时的原始配套规格。

3. **产品资产 (Product Instance / Installed Base)**:
   - 物流/售后视角。对应唯一的序列号 (SN)。
   - 包含生产批次、品级（A/B/C）、资产当前状态、物主归属。
   - 所有的工单、保修判定、维修历史均挂载于此层级。

**业务流程闭环**:
- **创建**: 生产入库时，选择型号与 SKU，录入 SN，生成资产。
- **关联**: 工单创建时，通过 SN 自动关联资产及其背后的型号与规格。
- **追溯**: 在资产详情页可查阅该 SN 的全生命周期记录（从出厂、销售、注册到历次维修）。

### 1.5.7 保修计算引擎 (Warranty Engine)
...
##### 3.3.3.1 SN 状态与入库注册一体化

**业务背景**：系统初建，大量已销售设备尚未入库，需通过日常工单逐步登记。

**场景矩阵**：

| 场景 | SN 状态 | Header 显示 | 序列号区域 | 弹窗标题 |
|------|---------|-------------|-----------|---------|
| **A** | SN 在台账，已注册保修 | 🟢 在保 / 🔴 过保 | 仅显示 SN（可点击跳转） | - |
| **B** | SN 在台账，未注册保修 | 🟡 保修待确认 | [注册保修] 按钮 | 产品保修注册 |
| **C** | SN 不在台账 | 🟠 保修未知 | [入库并注册] 按钮 | 产品入库并注册保修 |
| **D** | SN 输入错误 | - | 允许继续，后续编辑修正 | - |

**数据流**：
```
工单创建 → 输入 SN
    ↓
/api/v1/context/by-serial-number 查询
    ↓
┌─────────────────┬─────────────────┬─────────────────┐
│ 找到设备+有保修 │ 找到设备+无保修 │ 未找到设备      │
│ (场景 A)        │ (场景 B)        │ (场景 C)        │
├─────────────────┼─────────────────┼─────────────────┤
│ product_id=设备ID│ product_id=设备ID│ product_id=NULL │
│ 显示保修状态    │ 显示[注册保修]  │ 显示[入库并注册]│
└─────────────────┴─────────────────┴─────────────────┘
              ↓ 点击按钮后 ↓
    ProductWarrantyRegistrationModal 弹窗
    (后端 /api/v1/products/register-warranty)
              ↓
    自动创建产品（如需）+ 注册保修
              ↓
    自动关联工单 product_id + 刷新界面
```

**权限控制**：
- [入库并注册] / [注册保修] 按钮：仅 MS 部门 + Admin 可见
- OP 部门：仅查看状态，不可操作入库/注册
...
### Phase 8: 洞察与报告 📊 ✅ **部分已完成 (v12.3.171)**

**目标**：数据驱动决策与流程收尾优化

**8.0 流程收尾重组 (Closing Process Restructuring)** ✅
- [x] **结案详情弹窗重组**: 引入横向 Tab 布局（维修总结/附件/结案交接），解决纵向过长导致的交互困难。
- [x] **UI 稳定性增强**: 固定 Body 最小高度 (520px)，消除 Tab 切换时的尺寸抖动。
- [x] **后端鉴权自愈**: 修正 `department_code` 映射逻辑，确保 MS 处理人权限校验精准。
- [x] **安全性加固**: 详情接口增加正整数 ID 校验，预防无效请求及 500 崩溃。
...
**当前版本**: 0.14.2
**最后更新**: 2026-03-15
**文档状态**: v12.3.268 — **工单创建活动详情增强**：支持查看完整创建信息（客户、产品、问题描述、附件）；**附件管理**：新增工单附件上传/删除 API，支持编辑时管理附件；**权限扩展**：MS Lead 可查看工单创建更正按钮；**附件查询修复**：工单详情现在正确返回所有附件（包括活动关联附件）。
