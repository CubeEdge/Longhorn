...
> **v0.12.5 重要更新**：
> - 修复经销商 Dashboard 工单统计显示 0000 问题
>   - 问题原因：前端查询工单时添加了 `keyword` 参数（经销商名称），但工单中的 `customer_name` 是终端客户名称，导致过滤后无匹配结果
>   - 解决方案：移除 `keyword` 参数，仅使用 `dealer_id` 查询经销商提交的工单
>   - 数据模型说明：工单 `account_id` 关联终端客户，`dealer_id` 关联提交工单的经销商
>
> **v0.12.4 重要更新**：
> - 章节顺序调整：1.3「客户与服务主体架构」与 1.4「三层工单模型」交换位置
>   - 先介绍客户/账户架构，再介绍工单模型，逻辑更顺畅
>
> **v0.12.3 重要更新**：
> - 1.4 章节重构：合并原 1.4「用户与服务职责」和 1.7「客户-联系人双层架构」
>   - 简化冗余内容，减少约 200 行
>   - 统一账户类型定义（DEALER/ORGANIZATION/INDIVIDUAL/INTERNAL）
>   - 优化章节结构，与 2.3 实现保持一致
>
> **v0.12.2 重要更新**：
> - 2.3 章节UI优化：
>   - 客户列表：移除「更多」菜单，改为直接「编辑」按钮
>   - 客户详情页：保留「更多」菜单，经销商显示3个动作（编辑/删除/停用），终端客户显示2个动作（编辑/删除）
>   - 经销商等级显示：列表和详情页显示 dealer_level（一级/二级/三级）而非 service_tier
>
> **v0.12.1 重要更新**：
> - 2.3 章节架构调整：
>   - 账户类型：CORPORATE → ORGANIZATION
>   - 所有账户类型（DEALER/ORGANIZATION/INDIVIDUAL）均支持多联系人
>   - 新增客户类型转换：INDIVIDUAL → ORGANIZATION
>   - 调整档案模块入口优先级：渠道和经销商为第一入口
> 
> **v0.12.0 重要更新**：
> - 新增 1.7 章节：客户-联系人双层架构，引入 Account + Contact 模型
> - 重构客户管理：从单层客户模型升级为"账户+联系人"双层架构
> - 支持B2B场景：解决企业客户多联系人、人员离职交接等问题
...
| 层级 | 角色 | 人员 | 职责 | 数据访问范围 |
|-----|------|------|------|-------------|
| **内部** | 市场部 | Effy(海外)、李雨健(国内)、Sherry/Cathy(负责人) | 工单处理、客户沟通、异常审批 | 全部工单、全部知识库 |
| **内部** | 生产部 | 黄碧珊(负责人)、陈高松/张工/张平娇 | 维修执行、报告审核 | 全部工单(维修视角)、内部知识库 |
| **内部** | 研发部 | 吴琪萌(产品)、时春杰(软件) | 问题分析、功能规划 | 全部工单(分析视角)、内部知识库 |
| **内部** | 管理层 | Jihua、Sherry | 决策、结算审核 | 全部数据 + 统计报表 |

本系统开放计划：1.0 内部；2.0经销商；3.0外部
...
**P1/P2/P3：服务响应优先级**（市场部首次响应时限）

| 优先级 | 响应时限 | 触发条件 |
|-------|---------|---------|
| **P1 紧急** | 8小时内 | 新机故障、批量问题、重要客户、影响拍摄 |
| **P2 高** | 24小时内 | VIP客户、核心功能不可用、经销商催办 |
| **P3 标准** | 48小时内 | 常规咨询、一般问题 |

**R1/R2/R3：返修处理优先级**（生产部维修时限）

| 优先级 | 完成时限 | 触发条件 | 备机申请 |
|-------|---------|---------|---------|
| **R1 加急** | 1个工作日 | 新机换新、紧急项目 | - |
| **R2 优先** | 3个工作日 | VIP客户、批量问题 | VVIP/机构客户可申请备机 |
| **R3 标准** | 7个工作日 | 常规返修 | - |

**Loaner Gear（备机）触发逻辑**：
- **触发条件**：VVIP/机构客户 + R2优先维修（预估3个工作日）
...
> **设计背景**：在专业的 B2B/Prosumer（专业消费者）服务系统中，如果把"单位"和"人"混在一起记录，后期会出现巨大的混乱。
> 
> **解决方案**：采用 "账户(Account) + 联系人(Contact)" 的双层结构，区分法律/商业实体与具体自然人。

#### 1.3.1 核心概念定义

| 概念 | 定义 | 关键属性 | 示例 |
|-----|------|---------|------|
| **账户 (Account)** | 法律/商业实体：公司、机构、个体户或个人 | 客户等级、信用额度、行业标签、资产列表(SN) | CVP UK, Netflix, 张三工作室, 李四(个人摄影师) |
| **联系人 (Contact)** | 具体的自然人，负责对接沟通 | 邮箱、电话、职位、语言偏好、状态 | Mike(CVP维修主管), Sarah(Netflix制片), 李四 |

#### 1.3.2 账户类型与权限

| 类型 | 分类 | 说明 | 典型场景 | 数据访问范围 |
|-----|------|------|---------|-------------|
| **DEALER** | 渠道和经销商 | 经销商 | ProAV, Gafpa, 1SV等 | 自己客户的工单、Dealer知识库、配件库存管理 |
| **ORGANIZATION** | 客户档案 | 机构客户 | 租赁公司、制作公司、广电、教育机构 | 仅自己的工单、Public知识库 |
| **INDIVIDUAL** | 客户档案 | 个人客户 | 个人摄影师、自由职业者、小型工作室 | 仅自己的工单、Public知识库 |
| **INTERNAL** | 内部/合作伙伴 | 友商、供应商、媒体等 | 内部账号 | 按部门权限 |

**经销商分级**：

| 等级 | 配件库存 | 维修能力 | 典型代表 |
|-----|---------|---------|---------|
| **一级经销商** | 有 | 强 | ProAV, Gafpa, 1SV, EU Office |
| **二级经销商** | 无 | 有 | DP Gadget |
| **三级经销商** | 无 | 无 | RMK |
...
```
情况：CVP UK 是一家大公司，有 Mike(维修) 和 Sarah(财务) 两个人。
...
│ 行业: [RENTAL_HOUSE]                    │
├─────────────────────────────────────────┤
│ 联系人1: Mike Johnson (PRIMARY)         │
│   - 职位: 维修主管                      │
│   - 邮箱: mike@cvp.uk                   │
│                                         │
│ 联系人2: Sarah Williams (ACTIVE)        │
│   - 职位: 财务经理                      │
│   - 邮箱: sarah@cvp.uk                  │
└─────────────────────────────────────────┘
...
工单表现：
- Mike 提了一个 RMA
- 工单归属：CVP UK (资产历史记在公司头上)
- 工单联系人：Mike (邮件发给 Mike)
- 财务结算：关联到 Sarah
```
...
注意：虽然名字一样，但数据库里是两条记录。
这样设计是为了系统架构统一，个人用户也能享受完整的
账户级功能（如设备管理、历史追溯等）。
```
...
```
情况：CVP 的 Mike 离职了，新来了 Tom 接手他的工作。
...
✅ 正确做法：
   1. 将 Mike 的 Contact 状态设为 INACTIVE
   2. 新建 Contact Tom，关联到 CVP UK

结果：
- 查去年的工单，还能看到是 Mike 处理的
- 新工单自动发给 Tom
- 资产历史保持完整
- 人员变动可追溯
```
...
**FAQ条目**:
- **内外部答案分离**: `external_answer`(客户/经销商可见) / `internal_answer`(仅内部)
- **关联信息**: 产品列表、固件版本、标签
- **媒体支持**: 图片、视频
- **UI安全机制**: 默认只复制外部答案,内部答案需二次确认
...
档案和基础信息 (Archives)
├── 1. 渠道和经销商 (第一入口)
│   ├── 经销商 (DEALER) - ProAV, Gafpa, 1SV等
│   └── 经销商备件库存 (Dealer Inventory)
├── 2. 客户档案 (Accounts) (第二入口)
│   ├── 机构客户列表 (ORGANIZATION) - 租赁公司、制作公司、广电、教育机构
│   └── 个人客户列表 (INDIVIDUAL) - 个人摄影师、自由职业者、小型工作室
└── 3. 资产和物料 (第三入口)
    ├── 设备资产 (Installed Base)
    └── 物料与价目 (Materials & Pricing)
```

**入口优先级说明**:
- **渠道和经销商** 作为第一入口，突出经销商在服务闭环中的核心地位
- **客户档案** 作为第二入口，区分机构客户和个人客户
- **资产和物料** 作为第三入口，提供设备和配件数据支撑

#### 2.3.2 账户类型定义

| 类型 | 分类 | 说明 | 典型场景 | 多联系人 |
|-----|------|------|---------|---------|
| **DEALER** | 渠道和经销商 | 经销商 | ProAV, Gafpa, 1SV等 | 支持 |
| **ORGANIZATION** | 客户档案 | 机构客户 | 租赁公司、制作公司、广电、教育机构 | 支持 |
| **INDIVIDUAL** | 客户档案 | 个人客户 | 个人摄影师、自由职业者、小型工作室 | 支持（可扩展） |

**类型转换说明**:
- INDIVIDUAL 可升级为 ORGANIZATION（当个人客户发展为小型制作公司时）
- 类型转换时保留所有历史数据（工单、设备、联系人）
- 转换后原联系人自动迁移，可继续添加新联系人
- **DEALER 不参与类型转换**（经销商和客户是不同业务实体）

#### 2.3.3 联系人模型 (Contact)

所有账户类型（DEALER/ORGANIZATION/INDIVIDUAL）均支持多联系人管理。
...
| email | string | 邮箱 |
| phone | string | 电话 |
| status | enum | ACTIVE/INACTIVE/PRIMARY |
| is_primary | boolean | 是否主要对接人 |
...
#### 2.3.4 渠道和经销商 (Dealers)

**功能入口**：`/archives/dealers`

**作为第一入口，突出经销商在服务闭环中的核心地位**
...
│ 1SV (美国)   | 一级经销商 | 维修单: 12 | 库存: 充足     │
│ DP Gadget    | 二级经销商 | 维修单: 8  | 库存: 无      │
└─────────────────────────────────────────────────────────┘
```
...
│ 联系人列表:                                      [+ 添加]│
│ ┌─────────────────────────────────────────────────────┐ │
│ │ Mike Johnson (PRIMARY)  维修主管  mike@proav.co.uk  │ │
│ │ Sarah Williams (ACTIVE) 财务经理  sarah@proav.co.uk │ │
│ └─────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────┤
│ Service Dashboard:                                      │
...
```
┌─────────────────────────────────────────────────────────┐
│ 筛选: [全部 ▼] [ORGANIZATION ▼] [搜索客户名称...] [+ 新增]│
├─────────────────────────────────────────────────────────┤
│ 北京电影学院 | 机构客户 | 设备: 12台 | 工单: 5 | VIP  [编辑]│
│ CVP UK       | 机构客户 | 设备: 45台 | 工单: 23| VIP  [编辑]│
│ John Smith   | 个人客户 | 设备: 2台  | 工单: 3 | 标准  [编辑]│
│ 王五工作室   | 个人客户 | 设备: 1台  | 工单: 1 | 标准  [编辑]│
└─────────────────────────────────────────────────────────┘
```

**列表页操作按钮规则**：
- 客户列表每行显示直接的「编辑」按钮，点击打开编辑弹窗
- 不显示「更多」菜单，简化列表操作
- 点击行其他区域进入客户详情页
...
│ 联系人列表:                                      [+ 添加]│
│ ┌─────────────────────────────────────────────────────┐ │
│ │ 张明 (PRIMARY)    设备管理主任   zhang@bfa.edu.cn   │ │
│ │ 李华 (ACTIVE)     技术工程师     li@bfa.edu.cn      │ │
│ │ 王强 (INACTIVE)   已离职         wang@bfa.edu.cn    │ │
│ └─────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────┤

**详情页操作按钮规则**：
- 详情页头部显示「更多」下拉菜单按钮
- 机构/个人客户：显示「编辑」「删除」2个动作
- 经销商：显示「编辑」「删除」「停用经销商」3个动作
- 「删除」和「停用」仅管理员可见

├─────────────────────────────────────────────────────────┤
│ 设备列表:                                        [+ 关联]│
│   MAVO Edge 8K (ME_207890) | 在保至2027-03-15          │
│   MAVO Edge 6K (ME_207891) | 在保至2027-03-15          │
├─────────────────────────────────────────────────────────┤
│ 历史工单:                                               │
...
│ 联系人列表:                                      [+ 添加]│
│ ┌─────────────────────────────────────────────────────┐ │
│ │ 王五 (PRIMARY)    摄影师         wang@example.com   │ │
│ │ 李四 (ACTIVE)     助理           li@example.com     │ │
│ └─────────────────────────────────────────────────────┘ │
│ 注: 个人客户也可添加多个联系人，适合小型工作室场景       │
├─────────────────────────────────────────────────────────┤
│ 设备列表:                                        [+ 关联]│
│   MAVO Edge 8K (ME_207890) | 在保至2027-03-15          │
├─────────────────────────────────────────────────────────┤
│ [升级为客户机构]                                        │
│ 当业务发展需要时，可将个人客户升级为客户机构，            │
│ 保留所有历史数据和联系人。                              │
└─────────────────────────────────────────────────────────┘
```
...
**客户类型选择逻辑**:
- 选择 **DEALER**: 显示经销商等级、维修能力字段；如果是在客户界面进行新增或者修改，隐藏Dealer选项；如果是在经销商界面进行处理，则客户类型默认是Dealer，并且不能修改，隐藏其他两个选项。
- 选择 **ORGANIZATION**: 显示行业标签、地址字段
- 选择 **INDIVIDUAL**: 简化字段，但仍支持多联系人
- **所有类型均支持多联系人管理**

**编辑账户表单**：
- 支持添加/删除/编辑联系人
- 标记联系人状态（ACTIVE/INACTIVE/PRIMARY）
- 设置主要对接人
- **客户类型转换**（仅 INDIVIDUAL → ORGANIZATION）
  - 保留所有历史数据
  - 保留所有联系人
  - 解锁机构客户专属字段（行业标签、地址等）

#### 2.3.8 设备资产 (Installed Base)
...
├── account_name: 账户名称
├── contact_name: 联系人姓名
├── contact_title: 联系人头衔 (如: "BBC纪录片导演")
