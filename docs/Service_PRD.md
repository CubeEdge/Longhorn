...
| 权限级别 | 可见内容 | 适用角色 |
|---------|----------|----------|
| Public | 技术规格、FAQ、功能介绍 | 所有人（含客户） |
| Dealer | Public + 经销商培训资料 | 经销商、内部员工 |
| Internal | Dealer + 维修手册、内部通告 | 内部员工 |
| Department | Internal + 部门专属知识 | 特定部门（如生产部维修指南） |
...
> 保留原4.1节的完整多维度查询能力，移动到3.4。

（保持原4.1.1-4.1.3的内容不变，仅调整序号为3.4.1-3.4.3）
