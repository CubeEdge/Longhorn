...
系统采用简化的**两层服务模型**，以服务记录为入口，RMA为物理返修出口：

```
┌─────────────────────────┐                 ┌─────────────────────────┐
│     服务记录 (SR)        │    需物理返修    │     RMA返修 (RA)         │
│                         │ ──────升级────▶ │                         │
│  SRD-YYMM-XXX (经销商)   │                 │  RA{产品}{渠道}-YYMM-XXX │
│  SRC-YYMM-XXX (直客)     │                 │  如: RA09C-2602-001     │
├─────────────────────────┤                 ├─────────────────────────┤
│ • 咨询解答              │                 │ • 收货登记              │
│ • 问题排查              │                 │ • 检测诊断              │
│ • 远程协助              │                 │ • 维修执行              │
...
| 类型 | 格式 | 示例 | 说明 |
|------|------|------|------|
| 经销商服务记录 | SRD-YYMM-XXX | SRD-2602-001 | D=Dealer |
| 直客服务记录 | SRC-YYMM-XXX | SRC-2602-001 | C=Customer |
| RMA返修工单 | RA{PP}{C}-YYMM-XXX | RA09C-2602-001 | PP=产品码, C=渠道码 |

**编号规则**：
- YYMM：年份后两位 + 月份（如2602=2026年2月）
- XXX：月度序号，001-999为十进制，超过999自动转16进制(A00-FFF)
- 每月序号重置，最大容量4095条/月
...
├── id
├── record_number: 记录编号 (自动生成)
│   // 经销商渠道: SRD-YYMM-XXX (如: SRD-2602-001)
│   // 直客渠道:   SRC-YYMM-XXX (如: SRC-2602-001)
│   // 序号超999自动转16进制 (001-FFF，最大4095/月)
│
├── // 客户信息 (代客户服务模式下可选填写)
...
issues (工单/返修记录)
├── id
├── rma_number: RMA返修授权号 (自动生成)
│   // 格式: RA{产品代码}{渠道代码}-{YYMM}-{序号}
│   // 示例: RA09C-2602-001 (产品09=MAVO Edge 2, C=直客)
│   // 示例: RA09D-2602-001 (产品09=MAVO Edge 2, D=经销商)
│   // 序号超999自动转16进制 (001-FFF，最大4095/月)
