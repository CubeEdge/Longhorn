...
- **全量部署**: 执行 `./scripts/deploy.sh` 同步至生产环境 `mini`。
 ...
 - **全量清理部署 (--clean)**: 引入并强制执行了新的部署规范。在同步前主动清理远程站点的 `dist` 目录，确保前端静态资源没有任何过期或冲突残留。
 - **版本控制一致性**: 同步递增 `client` (v12.3.146) 与 `server` (v1.8.17) 版本，维持前后端协同基准。
 - **构建流程校验**: 本地 `tsc` & `vite build` 满负载校验通过，产物已成功分发至 `mini` 节点。
