...
### [WIKI-003] 知识库网页导入优化 (已完成)
- [x] 网页加载失败时停止导入并告知用户
- [x] 只提取正文panel，过滤侧边栏、二维码、banner等非内容元素
- [x] 去重重复图片
- [x] 点击Wiki菜单应显示首页而非文章详情

### [WIKI-004] 知识库深层清洗与 AI 翻译修复 (已完成)
- [x] 修复 `removeContentTitle` 作用域导致的翻译失效
- [x] 自动剥离 Main/Function 导航面包屑
- [x] 自动截断 Previous/Next 尾部导航
- [x] 修正 AI 标题翻译方法名
- [x] 处理 AI 返回完整 HTML 骨架的异常情况
- [x] 版本号 UI 改为两排显示

### [WIKI-005] 评论区清理与 Bokeh 术语优化 (已完成)
- [x] HTML 阶段分层截断评论区（强信号 5% / 普通信号 20%）
- [x] 移除"文章来源""免责声明"等版权行
- [x] Bokeh 提示词增加 Eagle=猎影 术语映射
- [x] Bokeh 提示词增加内容清洁规则（Rule 7）
