...
- **刷新机制 (Refresh Logic)**:
  - **启动策略**: 每次冷启动 App 时，静默检查并更新本地词库（目标缓存 100 词）。
  - **手动刷新**: 支持 UI 强制刷新（Settings 或 Pull-to-refresh），触发立即重新拉取。
  - **进度反馈**: 加载过程中应提供非阻塞的进度提示 (Toast 或通过 Badge 变化)。
- **数据指标 (Data Metrics)**:
  - **当前规模**: ~317 词 (De:85, En:69, Ja:75, Zh:88)。
