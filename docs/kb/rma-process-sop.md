...
这个阶段就是进度条上的 **Step 5 结算审核**，它内部可能有两次传球（仅限付费单）：

### A. 财务核销 (ge_review) - 仅限非保修单：

*   **球的位置**：GE 财务池。
*   **责任人**：财务 Cathy 看到银行流水，认领工单，核对金额。
*   **节点结束**：Cathy 点击 **[ 确认收款结转 ]**，并填入实收金额和渠道。
*   **传球**：节点变为 `ms_closing`，球传给客服 **Effy**。

### B. 商务终审 (ms_closing)：

*   **球的位置**：Effy 的 My Tasks。
*   **操作**：Effy 做最后的"守门"，对保修单直接确认放行；对付费单，在此之前先点【最终确认】将球踢给GE（见环节A），等GE处理完退回后，再核对回寄地址、释放发货权。
*   **节点结束**：Effy 点击完成审核。
*   **下一节点**：系统自动将 `current_node` 设为 `op_shipping`（已发货），并将 `assignee_id` 设为 NULL（丢回 OP 物流发货池）。

---

## 核心机制：乒乓派单与防外泄规则 (自动流转)

### 1. 自动记号（乒乓球回传）
当你在一个节点完成操作后，如果下一个节点的归属部门，与你之前互动过的某人所在部门一致，系统会**优先把球直接传给那个人**。
*   **例子**：技术大李修完机器，球回传 MS 时，系统会找回当初负责审核的 Effy，而不是掉回 MS 公共池让别人重新看。

### 2. 部门默认收件人 (全科室分发)
如果找不到具体的历史互动人，但该部门（如 OP 售后库房）设置了“默认网捕”负责人，球会自动传给该默认负责人，防止遗漏。

### 3. 公共池兜底 (NULL 状态)
如果既没有历史互动人，也没有设置默认接球员，此时 `assignee_id` 变为 `NULL`。系统会将这批工单打上 **“🚩待认领”** 标签，放入该部门的【待分配池 (Unassigned Pool)】。
*   该部门的主管或成员可以自行前往 Team Hub 领取。

### 4. 严防外泄拦截 (Cross-Department Clearance)
**核心红线**：绝不允许 A 部门的阶段由 B 部门的人员负责。
*   当工单流向下个部门时，系统会进行强制校验。如果发现预期负责人属于其他部门，系统会大义灭亲地**清空此分配 (设为 NULL)**，让工单安全掉入目标部门的公共池，确保数据安全。因此，当你看到工单由于阶段跳转变成了“未指派”，这是一项保护机制，请本部门人员及时认领。
*   **节点结束**：Effy 点击 **[ 释放发货 ]**。
